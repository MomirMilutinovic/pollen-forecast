...
### Multivariate plots
```{r}
  correlationMatrix <- mydata %>% select(-next_day) %>% cor
...
```{r}
set.seed(8)
  trainPart <- createDataPartition(mydata$next_day, p=0.8, list=FALSE)
  train <- mydata[trainPart,]
  test <- mydata[-trainPart,]
...
```{r}
set.seed(8)
fit.xgbDART <- train(next_day ~ ., data=train, method='xgbDART', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
svmGrid <- data.frame(C=1:4*0.25)
fit.svmLinear <- train(next_day ~ ., data=train, method='svmLinear', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.rf <- train(next_day ~ ., data=train, method='rf', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"), tuneLength=10)

set.seed(8)
fit.knn <- train(next_day ~ ., data=train, method='knn', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"), tuneLength=15)

set.seed(8)
fit.glmboost <- train(next_day ~ ., data=train, method='glmboost', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.lm <- train(next_day ~ ., data=train, method='lm', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.treebag <- train(next_day ~ ., data=train, method='treebag', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.ridge <- train(next_day ~ ., data=train, method='ridge', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.bagEarth <- train(next_day ~ ., data=train, method='bagEarth', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.lmStepAIC <- train(next_day ~ ., data=train, method='lmStepAIC', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale"))

set.seed(8)
fit.mlp <- train(next_day ~ ., data=train, method='mlp', trControl = trainControl(method = "cv", number = 10), metric = 'RMSE', preProcess=c("center","scale")) 
