...
```{r}
  ...
  trainPart <- createDataPartition(mydata$next_day, p=0.6, list=FALSE)
  train <- mydata[trainPart,]
  test <- mydata[-trainPart,]
  validationPart <-  createDataPartition(test$next_day, p=0.5, list=FALSE)
  validation <- mydata[validationPart, ]
  test <- mydata[-validationPart,]
...
algorithms <- c('svmLinear', 'xgbDART', 'rf', 'knn', 'bayesglm', 'treebag')
...
baseline.models <- lapply(algorithms, trainModel, data = train)
```
...
```{r}
delta.models <- lapply(algorithms, trainModel, data = train)
```
...
```{r}
encodedDay.models <- lapply(algorithms, trainModel, data = train)
```

```{r}
  for (model in encodedDay.models) {
    print(model)
    cat("\n")
  }
```
